...
DIC.df <- data.frame(model = c("v1-1", "v1-2", "v1-3",
                               "v2-1", "v2-2", "v2-3", 
                               "v3-1", "v3-2", "v3-3"),
                     DIC = c(jm.v1_1$DIC, jm.v1_2$DIC, jm.v1_3$DIC, 
                             jm.v2_1$DIC, jm.v2_2$DIC, jm.v2_3$DIC, 
                             jm.v3_1$DIC, jm.v3_2$DIC, jm.v3_3$DIC),
                     Rhat = c(lapply(jm.v1_1$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(jm.v1_2$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(jm.v1_3$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(jm.v2_1$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(jm.v2_2$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(jm.v2_3$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(jm.v3_1$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(jm.v3_2$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(jm.v3_3$Rhat, max, na.rm = T) %>% unlist() %>% max())) %>%
  mutate(dDIC = DIC - min(DIC)) %>%
  arrange(dDIC)

```

THey seemed to converge okay; all maximum Rhat values are < 1.02. v2-1 is the best model among these. So, let's use that for cross validation etc. Take a look at posterior trace plots.


```{r jags-results-v1-1, echo=FALSE, message=FALSE}
jm <- jm.v2_1
mcmc_trace(jm$samples, c("p[1,1]", "p[1,2]", "p[1,4]", 
                         "p[2,1]", "p[2,2]", "p[2,4]",
...
Not very good... Why do they all have similar negative slopes? 
