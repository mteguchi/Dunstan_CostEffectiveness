...
Hatchlig counts were examined in the number per meter using the length of a trench (50 m vs 100 m). Furthermore, in order to consider the differences in the number of nesting females per nesting season, I also examined the number of hatchlings per meter per hour per 1000 adult females. The numbers of females were derived from vessel-based Petersen estimates in the most recent annual report (Figure \@ref(fig:Figure-females)).  

```{r data-import, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
n.females.df <- data.frame(year = c(2015:2022),
  ...
  mutate(n.females.scaled = scale(n.females))
...
```{r pivot-check, message=FALSE, echo=FALSE, warning=FALSE, cache=TRUE}
...
```{r raw-plot, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
legend_bottom <- get_legend(p.raw.counts + 
                              guides(color = guide_legend(nrow = 2)) +
                              theme(legend.position = "bottom", 
                                    legend.direction = "horizontal",
                                    legend.justification = "left"))

p.counts <- plot_grid(p.raw.counts + theme(legend.position = "none"), 
                      p.raw.counts.per.female + theme(legend.position = "none"),
                      legend_bottom,
                      labels = c("(1)", "(2)"),
                      rel_heights = c(1, 0.1))

# ggsave(filename = paste0("figures/raw_counts_", dpi.set, "dpi.png"),
#        plot = p.counts, device = "png", dpi = 600, bg = "white")
...
```{r sector-A, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
At sector A, only 6-hr surveys were conducted in `r min(data.1.A$year)` - `r max(data.1.A$year)`. The observed variability in the number of hatchlings among surveys in 2020 was large (Figure \@ref(fig:Figure-sector-A)). In 2021, even though the number of observed hatchlings was low, the number of hatchligs per 1000 females was higher than the other two years (Figure \@ref(fig:Figure-sector-A)), indicating a negative relationship between hatching success rates and the number of females. 
...
```{r sector-A2, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
In sector A2, 12-hr surveys were conducted from 2015 to 2018, whereas 6-hr surveys were conducted in 2019. Hourly counts of hatchlings reached greatest in the first 6 hrs, especially 3-4 hrs from the beginning of each survey (2100-2200) and declined subsequently (Figure \@ref(fig:Figure-sector-A2)).
...
```{r sector-B, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
In sector B, 12-hr surveys were conducted from 2015 to 2018, whereas 6-hr surveys were conducted from 2019 to 2022. As it was seen for the sector A2, hourly counts of hatchlings reached greatest in the first 6 hrs, especially 3-4 hrs from the beginning of each survey (2100-2200) and declined subsequently for 2016 and 2018 (Figure \@ref(fig:Figure-sector-B)). In other years (e.g., 2015 and 2017), however, no obvious peaks were observed. The number of hatchlings was higher during the first half of survey than the second half. 
...
```{r sector-D, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
```{r sector-2014S, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
In sector 2014S, 12-hr surveys were conducted from 2015 to 2018, whereas 6-hr surveys were conducted from 2019 to 2022 (Figure \@ref(fig:Figure-sector-2014S)). Similarly to other sectors, peaks were found in the first few hours of survey. In 2016, there was a second peak at 5-6 hrs into the survey. In general, higher counts were recorded in the first half (1800-2400) of the night.  
...
### Sector 2017S {-}
```{r sector-2017S, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
### Sector 2019S {-}
```{r sector-2019S, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
### Sector 2019N {-}
```{r sector_2019N, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
In sector 2019N, 6-hr surveys were conducted in 2022. There was a peak at 2-3 hrs after 1800 (2000-2100; Figure \@ref(fig:Figure-sector-2019N)).
...
## Estimating the total number of hatchlings from 6-hr sampling {-}

More hatchlings are generally found before midnight at all sectors and years. To estimate the total number of hatchlings per night (1800 - 0600) from a reduced sampling design (1800 - 2400), two approaches were used: (1) the proportion in the number of hatchlings in the first 6 hr period and (2) a regression model to determine the relationship between the first and second halves.

### Proportions of hatchlings in the first half (1800-2400) {-}

```{r first-vs-second, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
The proportion of hatchlings that were counted during the first 6 hrs, i.e., 1800 to 2400, relative to the 12-hr period (1800-0600) ranged from `r signif(min(counts.1st.2nd$ratio), 3)` to `r signif(max(counts.1st.2nd$ratio), 3)`. Majority (`r counts.1st.2nd %>% filter(ratio > 0.5) %>% nrow()` of `r nrow(counts.1st.2nd)`) were greater than 0.5 (mean = `r signif(mean(counts.1st.2nd$ratio), 3)`, SE = `r signif(sqrt(var(counts.1st.2nd$ratio))/sqrt(nrow(counts.1st.2nd)), 3)`). Counts that resulted in low proportions of hatchlings during the first half of the evenings occurred in 2017 at the sectors A2, B, and 2014S, and in 2018 at the sector A2 (Figure \@ref(fig:Figure-ratio-sectors)).
...
```{r sector-year-summary, echo=FALSE, message=FALSE, cache=TRUE}
...
Variability in the proportions is smaller among sectors within a year than among years within each sector, except 2018, when the variability in the sector A2 was large (Figure \@ref(fig:Figure-ratio-years)). Standard errors within year ranged from `r signif(min(counts.1st.2nd.sectors$SE.sectors), 3)` to `r signif(max(counts.1st.2nd.sectors$SE.sectors), 3)`, whereas those within sector among yeras ranged from `r signif(min(counts.1st.2nd.years$SE.years), 3)` to `r signif(max(counts.1st.2nd.years$SE.years), 3)`.
...
### Estimating the total number of hatchligs using the proportions {-}

I used the proportion of hatchlings that were observed during the first half of the evening to estimate the total number of hatchlings. The proportions were computed in four different ways: (1) only the same sector for that year, (2) all sectors for that year, (3) all other sectors for that year, or (4) all sectors from other years. In order to test for the performance of the method and compare among the ways to calculate proportions, cross validations were conducted for all date- and sector-specific surveys that were 12 hr long.  

```{r cross-validation-1, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
The calculated numbers of hatchlings often agreed with the true values (Figure \@ref(fig:Figure-X-validation)). Calculated values were accurate for 2015, whereas they were inaccurate for 2017. Regardless of how the proportions were computed, calculated numbers were accurate when the number of hatchlings per meter was < 10. Calculated values were generally negatively biased when the number of hatchlings per meter was > approximately 10. 
...
In order to examine the accuracy of calculated values, differences between calculated and observed values were computed. Differences are zero when the method is accurate. 

```{r Figure-ratio, echo=FALSE, message=FALSE, fig.cap="The differences between the observed and calculated numbers of hatchlings using the proportion of the number of hatchlings in the first half of the evening. Calculations were conducted in a cross validation manner where each survey was withheld from computing the proportion to calculate the number of hatchlings in the second half of the evening. Colors indicate different sets of data to cross validate: orange = using the same sector within the same year, purple = using all sectors within the same year, red = using other sectors witin the same year, and yellow = using all sectors from other years. Data points were jittered to reduce the overlap."}

knitr::include_graphics(paste0("figures/ratio_difs_", dpi.set, "dpi.png"))

```

The comparison of the calculated values (i.e., hatchling counts during the second half of each survey) indicated that the using all sectors from all other years (yellow) resulted in smallest variability in the difference between the true and calculated values (Table \@ref(tab:Table-summary-ratio), Figure \@ref(fig:Figure-ratio)). It is important to note that using all sectors or one sector within the same year requires some 12-hr sampling to be conducted within each year. The only set that requires no 12-hr sampling in the future is "Other years" (yellow). Considering the logistical difficulties of conducting 12-hr sampling at all sectors, I compare overall hatchling estimates from "other sectors" and "other years" in the following section.  


```{r Table-summary-ratio, echo=FALSE, warning=FALSE}
flextable(ratio.summary.table) %>% 
  #hline(i = (nrow(ratio.summary.table)-1)) %>%
 set_caption(paste0("A comparison of data sets for cross validation to calculate the proportion of the number of hatchlings during the first half of the evening. Results in the table indicate the differences from the true values.")) %>%
...
### Estimating the unobserved counts from proportions {-}

In this section, I use the entire dataset and estimate the number of hatchlings for the second half of each evening when no survey was conducted from 2400 - 0600, using all available proportions from all years and all sectors. To estimate the uncertainty, I fitted the beta distribution to all available proportions and computed 0.025 and 0.975 quantiles.  

```{r proportion-estimates, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
counts.1st.2nd.all <- rbind(counts.1st.2nd.12hr, counts.1st.2nd.6hr) %>%
  mutate(counts_1_100m = counts_m_1 * 100,
         counts_2_100m = counts_m_2 * 100)
...
data.out$estim.2_100m <- data.out$counts_1_100m * mean.prop
data.out$estim.2_100m_q2.5 <- data.out$counts_1_100m * qtiles.fit[1]
data.out$estim.2_100m_q50 <- data.out$counts_1_100m * qtiles.fit[2]
data.out$estim.2_100m_q97.5 <- data.out$counts_1_100m * qtiles.fit[3]

p.estimated.prop <- ggplot(data = data.out) + 
  geom_point(aes(x = counts_1_100m, 
                 y = estim.2_100m, 
                 color = sector),
                 #size = duration_hr),
             alpha = 0.3) +
  geom_errorbar(aes(x = counts_1_100m,
                    ymin = estim.2_100m_q2.5,
                    ymax = estim.2_100m_q97.5,
                    color = sector),
                alpha = 0.6) +
  geom_point(aes(x = counts_1_100m, y = counts_2_100m),
             shape = 0) +
  geom_abline(slope = 1, intercept = 0)+
  facet_wrap(~ year) + labs(title = "Proportion") +
  xlab("Observed counts from 1800-2400") +
...
As it was seen from the previous analyses, observed and estimated counts were similar for 2015 and 2016. Departures were observed for 2017 and 2018. The subsequent years have no way of knowing how estimate values compared to true values (Figure \@ref(fig:Figure-ratio-estimates)). 

```{r Figure-ratio-estimates, echo=FALSE, message=FALSE, fig.cap="Estimated abundance and their 95% CI of hatchling counts during the second half of the night when the proportion of the number of hatchlings during the first half to the entire night was used. The solid lines indicate the y=x, which means the numbers of hatchlings before and after the midnight are the same. Hollow squares indicate observed values."}

knitr::include_graphics(paste0("figures/estimated_counts2_", dpi.set, "dpi.png"))

```

### Statistical modeling of hatchling counts {-}

In this section, I use a statistical model to estimate the number of hatchlings during the second half of the night ($m$). I treated the observed number of hatchlings from 1800 to 2400 ($n$) in the j-th sector of the k-th sampling day during the y-th year as a random deviate from a statistical distribution. Because survey distance ($L$) was changed from 100 m to 50 m, it was included in the model. When taking the natural log of the number of hatchlings per unit distance, 
...
First, I considered the Poisson distribution as the likelihood function for the observed counts ($n$ and $m$). However, it did not fit well according to the Pareto k statistics, which measure how well a statistical model fits to the observed data in the Bayesian context (ref here). Consequently, I altered the likelihood function to the normal distribution and transformed the observed counts in the natural logarithmic scale. In order to deal with observed zero, I added one to each count.
...
The number of hatchlings from 2400 to 0600 (m) (plus one) in the natural log space is also a normal random deviate and its mean is a function of $\lambda_{j,k,y}$ and the proportion of the number of hatchlings from 1800 to 2400 to that from 1800 to 0600 ($\theta_{j,k,y}$):

$$
ln(m_{j,k,y} +1) \sim N(\lambda_{j,k,y} + \theta_{j,k,y}, \sigma_m) 
$$

The mean ($\lambda_{j, k, y}$) is a function of covariates, including year ($Y$) and the number of females ($N_{female}$), the day of sampling (# days since Jan 1; $D_{j,k,y}$), and sector ($S_{j,y}$), which was treated as a random factor:
...
For the additive term ($\theta$), in the log scale, that converts the mean of the first half of the evening ($\lambda$) to the second half of the evening ($\lambda + \theta$), I looked at five variations:

1. year specific ($\theta_{y}$)

2. year and sector specific ($\theta_{j, y}$)

3. sector specific ($\theta_{j}$)

4. year, sector, and sampling day specific ($\theta_{j,k,y}$)
...
Parameters were estimated via the Bayesian approach using JAGS and the jagsUI package in R. I used the following prior distributions.

$\lambda_{j,k,y} \sim GAM(1, 0.1)$
...
jags.data <- list(n = log(counts.1st.2nd.jags.data$counts_1 + 1),
                  ...
                  n.years = length(unique(counts.1st.2nd.jags.data$year_num)),
                  n.sectors = length(unique(counts.1st.2nd.jags.data$sector_num)),
                  N.fem = as.vector(counts.1st.2nd.jags.data$n.females.scaled),
...
if (!file.exists(out.file.name)){
  ...
  for (k in 1:length(model.names)){
    ...
    jags.out[[k]] <- jags(jags.data,
                          ...
                          DIC = T, parallel=T, verbose = FALSE)
...
Almost all models converged fine according to Rhat values (Table \@ref(tab:Table-model-summary)). According to DIC values, the best model was `r LOOIC.df %>% filter(dDIC == 0) %>% select(model) %>% pull()`, which also was the best model according to LOOIC values for m. Pareto k statistics indicated that the model fit well to the data. This model treats the additive factor to be year-specific  ($\theta_{y}$) and common among sectors. The mean number of hatchlings before the midnight is a function of sector, the number of days since January 1st, and the number of estimated females for the year. According to the LOOIC values for $n$, which is the number of hatchlings in the first half of the evening, the best model was v1-6, which had year-specific $\theta$ and sectors and the number of females as covariates for the mean. The second best model was v5-6, which had a constant $\theta$ and sectors and the number of females as covariates for the mean. 


```{r Table-model-summary, echo=FALSE, warning=FALSE}
flextable(LOOIC.df %>% filter(dLOOIC.n < 10)) %>% 
  #hline(i = (nrow(ratio.summary.table)-1)) %>%
  set_caption(paste0("Comparison of how models fit to the hatchling count data. Only first 10 models are shown. Rhat indicates the conversion of Markov chain Monte Carlo, where Rhat < 1.1 is considered to be acceptable. dDIC indicates the difference in deviance information criteria (DIC) value from the smallest DIC value, where the model with smallest DIC is considered best. dLOOIC.m and dLOOIC.n indicate leave-one-out information criteria for m (the number of hatchlings in the second half of the evening) and n (the number of hatchlings in the first half of the evening), where the smaller values indicate better models. max.pareto.m and max.pareto.n indicate the maximum value of Pareto k statistic, which is an index of model fit. Values of Pareto k statistics less than 0.5 are considered acceptable. Values greater than 0.7 indicate ill fit of the model.")) %>%
  set_table_properties(width = 0.5, layout = "autofit") %>%
...
```{r p-trace-plots, echo=FALSE, message=FALSE, include=FALSE, cache=TRUE}
...
```{r S-trace-plots, echo=FALSE, message=FALSE, include=FALSE, cache=TRUE}
...
```{r muS-sS-trace-plots, echo=FALSE, message=FALSE, include=FALSE, cache=TRUE}
...
```{r sigma.n-B.N.fem-trace-plots, echo=FALSE, message=FALSE, include=FALSE, cache=TRUE}
...
```{r X-validation-1, echo=FALSE, message=FALSE, cache=TRUE, results='hide'}

cross.validation.v1_6.out <- cross.validataion.fcn(model.base.name = model.base.name, 
                                                   model.ver = "v1-6", 
                                                   jags.data = jags.data, 
                                                   jags.params = jags.params, 
                                                   MCMC.params = MCMC.params, 
...
cross.validation.v5_6.out <- cross.validataion.fcn(model.base.name = model.base.name, 
                                                   model.ver = "v5-6", 
                                                   jags.data = jags.data, 
                                                   jags.params = jags.params, 
                                                   MCMC.params = MCMC.params, 
...
model.ver <- "v1-6"
p.X_validation_V1_6 <- ggplot(cross.validation.v1_6.out$output) +
...
save.fig.fcn(p.X_validation_V1_6, 
             file.name = paste0("figures/Cross_validation_log_", 
                                model.ver, "_", dpi.set, "dpi.png"), 
             replace = F, dpi = dpi.set)

model.ver <- "v5-6"
p.X_validation_V5_6 <- ggplot(cross.validation.v5_6.out$output) +
  #geom_point(aes(x = sector, y = counts_2),
  #           color = "gold") +
  geom_point(aes(x = log(counts_2+1), 
                 y = estim.m, 
...
save.fig.fcn(p.X_validation_V5_6, 
             file.name = paste0("figures/Cross_validation_log_", 
                                model.ver, "_", dpi.set, "dpi.png"), 
             replace = F, dpi = dpi.set)
```
...
The cross validation process indicated that the model predicted missing values fairly well for 2015 (Figure \@ref(fig:Figure-X-validation-log-v1-6)), where observed and estimated values nearly lined up on the y = x line. Deviation from the line, however, was obvious for 2017 and 2018. It is important to note that v1-6 model has a year-specific $\theta$ values, which require more than one 12-hr sampling needs to be conducted annually in order for this model to work. V1-6 cannot be applied to the whole dataset because no data exist to estimate year-specific $\theta$ after 2018. 


```{r Figure-X-validation-log-v1-6, echo=FALSE, message=FALSE, fig.cap="The differences between the observed and calculated numbers of hatchlings using a statistical model (v1-1). Calculations were conducted in a cross validation manner where each survey was withheld from computing the ratio to be used in calculating the second half of the evening."}

knitr::include_graphics(paste0("figures/Cross_validation_log_v1-6_", dpi.set, "dpi.png"))

```


The best model with a common $\theta$ value was V5-6 (Table \@ref(tab:Table-model-summary)). The model fit well to the data according to the Pareto-k statistics, although it was ranked after V1 and V2 models. Results from cross validation indicated that the model estimated missing values well (Figure \@ref(fig:Figure-X-validation-log-v5-6)). Because this model has a common $\theta$ value, it can be used for years with no observations in the second half of the evening. 


```{r Figure-X-validation-log-v5-6, echo=FALSE, message=FALSE, fig.cap="The differences between the observed and calculated numbers of hatchlings using a statistical model (v5-1). Calculations were conducted in a cross validation manner where each survey was withheld from computing the ratio to be used in calculating the second half of the evening."}

knitr::include_graphics(paste0("figures/Cross_validation_log_v5-6_", dpi.set, "dpi.png"))
...
I applied the v5-6 model to the entire data set in order to estimate the counts from 2400 - 0600, when no surveys were conducted. 

```{r estimating-missing-counts, echo=FALSE, message=FALSE, warning=FALSE, results='hide', cache=TRUE}
...
jags.data <- list(n = log(counts.all.jags.data$counts_1 + 1),
                  ...
                  N.fem = as.vector(scale(counts.all.jags.data$n.females)),
                  N.fem.year = as.vector(n.females.df$n.females),
...
for (k in 1:length(model.names)){
  ...
  m.estim <- m.q2.5 <- m.q50 <- m.q97.5 <- vector(mode = "numeric", 
                                                  length = length(jags.data$m))
...
LOOIC.df <- data.frame(model = out.list$model.names,
                       ...
                       LOOIC.m = lapply(out.list$LOOIC.out.m, 
                                        FUN = function(x) 
                                          x$loo.out$estimates["looic", "Estimate"]) %>% 
                         unlist(),
                       max.pareto.m = lapply(out.list$LOOIC.out.m, 
                                             FUN = function(x) 
                                               x$loo.out$diagnostics$pareto_k %>%
                                               max()) %>%
                         unlist(),
                       LOOIC.n = lapply(out.list$LOOIC.out.n, 
                                        FUN = function(x) 
...
```{r estimated-m, echo=FALSE, message=FALSE, warning = FALSE, cache=TRUE}

counts.all.estims <- cbind(counts.all %>%
                             mutate(counts_1_100m = 100 * counts_1/length_m,
                                    counts_2_100m = 100 * counts_2/length_m,
                                    counts_2a_100m = 100 * counts_2a/length_m), 
                           data.frame(estim_m_v1.6 = out.list$m.estims[[1]]$m.estim,
                                      q2.5_m_v1.6 = out.list$m.estims[[1]]$m.q2.5,
                                      q50_m_v1.6 = out.list$m.estims[[1]]$m.q50,
                                      q97.5_m_v1.6 = out.list$m.estims[[1]]$m.q97.5,
                                      estim_m_v5.6 = out.list$m.estims[[2]]$m.estim,
                                      q2.5_m_v5.6 = out.list$m.estims[[2]]$m.q2.5,
                                      q50_m_v5.6 = out.list$m.estims[[2]]$m.q50,
                                      q97.5_m_v5.6 = out.list$m.estims[[2]]$m.q97.5)) %>%
  mutate(estim_m_100m_v1.6 = (exp(estim_m_v1.6) - 1) * 100/length_m,
         q2.5_m_100m_v1.6 = (exp(q2.5_m_v1.6) - 1) * 100/length_m,
         q50_m_100m_v1.6 = (exp(q50_m_v1.6) - 1) * 100/length_m,
         q97.5_m_100m_v1.6 = (exp(q97.5_m_v1.6) - 1) * 100/length_m,
         estim_m_100m_v5.6 = (exp(estim_m_v5.6) - 1) * 100/length_m,
         q2.5_m_100m_v5.6 = (exp(q2.5_m_v5.6) - 1) * 100/length_m,
         q50_m_100m_v5.6 = (exp(q50_m_v5.6) - 1) * 100/length_m,
         q97.5_m_100m_v5.6 = (exp(q97.5_m_v5.6) - 1) * 100/length_m)

# p.estimated.m.V1.6 <- ggplot(data = counts.all.estims) + 
#   geom_point(aes(x = counts_1_100m, 
#                  y = estim_m_100m_v1.6, 
#                  color = sector),
#                  #size = duration_hr),
#              alpha = 0.3) +
#   geom_errorbar(aes(x = counts_1_100m,
#                     ymin = q2.5_m_100m_v1.6,
#                     ymax = q97.5_m_100m_v1.6,
#                     color = sector),
#                 alpha = 0.6) +
#   geom_point(aes(x = counts_1_100m,
#                  y = counts_2a_100m), shape = 0) +
#   geom_abline(slope = 1, intercept = 0)+
#   facet_wrap(~ year) + labs(title = "v1-6") +
#   xlab("Observed counts from 1800-2400") +
#   ylab("Estimated counts from 2400-0600") +
#   theme(legend.position = "bottom")
# 
# save.fig.fcn(p.estimated.m.V1.6, 
#              file.name = paste0("figures/estimated_m_v1-6_", dpi.set, "dpi.png"), 
#              replace = T, dpi = dpi.set)
...
p.estimated.m <- ggplot(data = counts.all.estims) + 
  geom_point(aes(x = counts_1_100m, 
             ...
             alpha = 0.3) +
  geom_errorbar(aes(x = counts_1_100m,
                    ymin = q2.5_m_100m_v5.6,
                    ymax = q97.5_m_100m_v5.6,
                    color = sector),
                alpha = 0.6) +
  geom_point(aes(x = counts_1_100m,
                 y = counts_2a_100m), shape = 0) +
  geom_abline(slope = 1, intercept = 0)+
  facet_wrap(~ year) + labs(title = "v5-6") +
  xlab("Observed counts from 1800-2400") +
  ylab("Estimated counts from 2400-0600") +
  theme(legend.position = "bottom")

save.fig.fcn(p.estimated.m, 
             file.name = paste0("figures/estimated_m_v5-6_", dpi.set, "dpi.png"), 
             replace = T, dpi = dpi.set)

```


When estimating the number of hatchlings for the second half of the evening were estimated from the hatchling counts of the first half and data from other years, the variability is considerably less than observed counts during the second half of the evening (Figure \@ref(fig:Figure-estimated-counts-v5-6)). This result indicates that there are significant variability in the number of hatchlings that are counted during the second half of the evening.


```{r Figure-estimated-counts-v5-6, echo=FALSE, message=FALSE, fig.cap="Observed and estimated counts from 2400 - 0600 using the v5-6 model as a function of the counts from the first half of the evening. Hollow squares are observed counts. The solid lines indicate y=x."}

knitr::include_graphics(paste0("figures/estimated_m_v5-6_", dpi.set, "dpi.png"))

```

## Comparing the ratio-based and model-based estimates and conclusions {-}

The model-based estimation process can incorporate sampling variability. It requires additional assumptions (e.g., statistical distributions, additivity), however. Computing proportions is a simple process and requires less time computing.  

There was no clear winner between the two approaches. Surprisingly, using the simple proportion from 2015-2018 provided "reasonable" estimates of hatchling counts. Estimates were not always accurate, however. Furthermore, the proportion approach does not take into account of the sampling error. Observed values are used as the true numbers of hatchlings. Results of the model-based approach were not different from those from the proportion-based approach. 

For both approaches, as expected, the difference between the estimates and the true counts (when available) was greater when the number of hatchlings during the first half of the evening was large.

To estimate the total hatchling production at Raine Island, the 6-hr sampling of hatchling may be sufficient when the number of hatchlings is low. During a high nesting season, it may be necessary to conduct a few 12-hr sampling to better estimate the ratio or the correction factor for that year (and others). Because the ratio of the first half to the total appears to be very similar among sectors, only one sector needs to be sampled for 12 hrs if 12-hr sampling at multiple sectors is logistically challenging. 
