...
if (!file.exists(out.file.name)){
  ...
  for (k in 1:length(model.names)){
    print(paste( k, " of ", length(model.names)))
...
The following statement was when I used lambda x p as the mean, which I think is incorrect.
Almost all models converged fine according to Rhat values (Table \@ref(tab:Table-model-summary)). According to DIC values, the best model was `r LOOIC.df %>% filter(dDIC == 0) %>% select(model) %>% pull()`. The Rhat value for the model, however, was greater than ideal (`r LOOIC.df %>% filter(dDIC == 0) %>% select(Rhat) %>% pull()`). The second best model according to DIC values was `r LOOIC.df %>% filter(dDIC == sort(dDIC)[2]) %>% select(model) %>% pull()`, which was the best model according to LOOIC values for m. I use the model for cross validation below. This model treats the proportion ($p$) to be year-specific and common among sectors. The mean number of hatchlings before the midnight is a function of sector, the number of days since January 1st, and the number of estimated females for the year. 


The following statement is when I used lambda + p as the mean, which I think is correct.
Almost all models converged fine according to Rhat values (Table \@ref(tab:Table-model-summary)). According to DIC values, the best model was `r LOOIC.df %>% filter(dDIC == 0) %>% select(model) %>% pull()`. The model, however, was not considered "good" according to LOOIC values and its maximum Pareto stastistic was greater than 0.7, indicating ill fit to the data. The best model according to LOOIC values for $m$ was `r LOOIC.df %>% filter(dLOOIC.m == 0) %>% select(model) %>% pull()`. 
