...
How to use these ratios using cross validation...
1. Estimate the second half of each survey using the ratio from the rest. 
2. Estimate the second half of each survey using the ratio from other years. 

NEED TO CHANGE THE DATE_NUM TO ACTUAL SEQUENCE, THEY ARE NOT... THEY NEED TO BE IN 1 TO 50.

```{r cross-validation-1, echo=FALSE, message=FALSE}
# Create a sequential ID number for each survey
data.1 %>%
  mutate(date.seq = date_num - min(date_num) + 1) -> data.1

# first, I use all surveys except one. 
n.surveys <- length(unique(data.1$date.seq))

k <- 1
for (k in 1:n.surveys){
  data.1.1 <- data.1 %>% filter(date.seq != k)
}
