...
if (!file.exists(cross.validation.out.file.name)){
  ...
  for (k in 1:nrow(counts.1st.2nd.jags.data)){
    ...
    counts.X.val$estim.X.val[k] <- exp(jm$mean$m[k])
    counts.X.val$q2.5.X.val[k] <- exp(jm$q2.5$m[k])
    counts.X.val$q50.X.val[k] <- exp(jm$q50$m[k])
    counts.X.val$q97.5.X.val[k] <- exp(jm$q97.5$m[k])
