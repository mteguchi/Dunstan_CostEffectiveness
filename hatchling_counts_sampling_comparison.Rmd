...
Parameters were estimated via the Bayesian approach using JAGS (Plummer Yr) and the jagsUI package (ref) in R (ref). I used the following prior distributions.
...
model.ver <- "v5-1"
p.X_validation_V5_1 <- ggplot(cross.validation.v5_1.out$output) +
  #geom_point(aes(x = sector, y = counts_2),
  #           color = "gold") +
  geom_point(aes(x = log(counts_2+1), 
                 y = estim.m, 
...
save.fig.fcn(p.X_validation_V5_1, 
             file.name = paste0("figures/Cross_validation_log_", 
                                model.ver, "_", dpi.set, "dpi.png"), 
             replace = F, dpi = dpi.set)
```
...
The cross validation process indicated that the model predicted missing values fairly well (Figure \@ref(fig:Figure-X-validation-log-v1-6)), where observed and estimated values nearly lined up on the y = x line. Deviation from the line, however, was obvious for 2017 and 2018. It is important to note that v1-6 model has a year-specific $\theta$ values, which require more than one 12-hr sampling needs to be conducted annually in order for this model to work. 
...
counts.all.estims <- cbind(counts.all %>%
  ...
  mutate(estim_m_100m_v1.1 = (exp(estim_m_v1.1) - 1) * 100/length_m,
         q2.5_m_100m_v1.1 = (exp(q2.5_m_v1.1) - 1) * 100/length_m,
         q50_m_100m_v1.1 = (exp(q50_m_v1.1) - 1) * 100/length_m,
         q97.5_m_100m_v1.1 = (exp(q97.5_m_v1.1) - 1) * 100/length_m,
         estim_m_100m_v5.1 = (exp(estim_m_v5.1) - 1) * 100/length_m,
         q2.5_m_100m_v5.1 = (exp(q2.5_m_v5.1) - 1) * 100/length_m,
         q50_m_100m_v5.1 = (exp(q50_m_v5.1) - 1) * 100/length_m,
         q97.5_m_100m_v5.1 = (exp(q97.5_m_v5.1) - 1) * 100/length_m)
...
p.estimated.m <- ggplot(data = counts.all.estims) + 
  geom_point(aes(x = counts_1_100m, 
             ...
             alpha = 0.3) +
  geom_errorbar(aes(x = counts_1_100m,
                    ymin = q2.5_m_100m_v5.1,
                    ymax = q97.5_m_100m_v5.1,
                    color = sector),
                alpha = 0.6) +
  geom_point(aes(x = counts_1_100m,
...
When estimating the second half of the evening from the hatchling counts of the first half and data from other years, the variability is considerably less than if the counts were made during the second half of the evening (Figure \@ref(fig:Figure-estimated-counts-v5-1)). 
...
The model-based estimation process requires a considerable effort. Computing proportions is a lot simpler and less time consuming. Here, I compare the two methods and discuss pros and cons of two approaches and recommendations for future sampling effort. 

There was no clear winner in the comparison. Surprisingly, using the simple proportion from 2015-2018 provided "reasonable" estimates of hatchling counts. Estimates were not always accurate, however. The proportion approach does not take into account of the sampling error. Observed values are used as the true numbers of hatchlings. Results of the model-based approach were not different from those from the proportion-based approach. 

For both approaches, the difference between the estimates and the true counts (when available) was greater when the number of hatchlings during the first half of the evening was large.

To estimate the total hatchling production on Raine Island, the 6-hr sampling of hatchling may be sufficient when the number of hatchlings is low. During a high nesting season, it may be necessary to conduct a few 12-hr sampling to better estimate the ratio or the correction factor for that year (and others). Because the ratio of the first half to the total appears to be very similar among sectors, only one sector needs to be sampled for 12 hrs if 12-hr sampling at multiple sectors is logistically challenging. 
