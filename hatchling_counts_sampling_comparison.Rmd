...
```{r Figure-X-validation, echo=FALSE, message=FALSE, fig.cap = "Observed and predicted numbers of hatchlings }
...
# v1 has year specific proportion p. v2 has year and sector specific p
# Convergence didn't reach for v1. So, I'm just using v2
...
# vk-4 has no sector random effects. Just # females and # days.

# v1 has year specific proportions (p[year])

if (!file.exists("RData/jags_out_hatchling_counts.rds")){
  start.time <- now()
  model.ver <- "v1-1"
  model.file <- paste0("models/model_counts_estimation_", model.ver, ".txt")
  jm.v1_1 <- jags(jags.data,
                  inits = NULL,
  ...
  # v2 has sector and year specific proportions (p[year, sector])
  model.ver <- "v2-1"
  model.file <- paste0("models/model_counts_estimation_", model.ver, ".txt")
  jm.v2_1 <- jags(jags.data,
                  inits = NULL,
                  parameters.to.save= jags.params,
                  model.file = model.file,
                  n.chains = MCMC.params$n.chains,
  ...
  # v3 has sector specific proportions (p[sector])
  model.ver <- "v3-1"
  model.file <- paste0("models/model_counts_estimation_", model.ver, ".txt")
  jm.v3_1 <- jags(jags.data,
                  inits = NULL,
                  parameters.to.save= jags.params,
                  model.file = model.file,
                  n.chains = MCMC.params$n.chains,
  ...
  # v4 has sector-year-day specific proportions (p[year, sector, day])
  model.ver <- "v4-1"
  model.file <- paste0("models/model_counts_estimation_", model.ver, ".txt")
  jm.v4_1 <- jags(jags.data,
                  inits = NULL,
                  parameters.to.save= jags.params,
                  model.file = model.file,
                  n.chains = MCMC.params$n.chains,
  ...
  saveRDS(out.list, file = "RData/jags_out_hatchling_counts.rds")
} else {
  out.list <- readRDS("RData/jags_out_hatchling_counts.rds")
}

DIC.df <- data.frame(model = c("v1-1", "v1-2", "v1-3", "v1-4",
                               "v2-1", "v2-2", "v2-3", "v2-4",
                               "v3-1", "v3-2", "v3-3", "v3-4",
                               "v4-1", "v4-2", "v4-3", "v4-4"),
                     DIC = c(out.list$v1_1$DIC, out.list$v1_2$DIC, 
                             out.list$v1_3$DIC, out.list$v1_4$DIC, 
                             out.list$v2_1$DIC, out.list$v2_2$DIC, 
                             out.list$v2_3$DIC, out.list$v2_4$DIC, 
                             out.list$v3_1$DIC, out.list$v3_2$DIC, 
                             out.list$v3_3$DIC, out.list$v3_4$DIC,
                             out.list$v4_1$DIC, out.list$v4_2$DIC, 
                             out.list$v4_3$DIC, out.list$v4_4$DIC),
                     Rhat = c(lapply(out.list$v1_1$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v1_2$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v1_3$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v1_4$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v2_1$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v2_2$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v2_3$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v2_4$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v3_1$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v3_2$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v3_3$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v3_4$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v4_1$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v4_2$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v4_3$Rhat, max, na.rm = T) %>% unlist() %>% max(),
                              lapply(out.list$v4_4$Rhat, max, na.rm = T) %>% unlist() %>% max())) %>%
...
They seemed to converge okay; all maximum Rhat values are < 1.02. v4-1 is the best model among these. So, let's use that for cross validation etc. Take a look at posterior trace plots.
...
flextable(DIC.df) %>% 
 ...
 set_caption(paste0("Model comparison.")) %>%
  set_table_properties(width = 0.5, layout = "autofit") %>%
  colformat_double(j = c("DIC", "Rhat", "dDIC"), digits = 2)

```



```{r jags-results-v4-1, echo=FALSE, message=FALSE}
jm <- out.list$v4_1
mcmc_trace(jm$samples, c("p[1,1,1]", "p[1,1,2]", "p[1,1,3]", 
...
if (!file.exists(paste0("RData/cross-validation_out_", model.ver, ".rds"))){
  start.time <- now()
  ...
  for (k in 1:nrow(counts.1st.2nd.jags.data)){
    ...
    out.list <- create.n.m.D.length(n.years, 
                                    n.sectors, 
                                    n.days, 
                                    x.val.jags.data)
    ...
    counts.X.val$q50.X.val[k] <- q50.x[dif.x > 0] %>% na.omit()
  ...
  saveRDS(cross.validation.out,
          file = paste0("RData/cross-validation_out_", model.ver, ".rds"))  
} else {
  
  cross.validation.out <- readRDS(file = paste0("RData/cross-validation_out_", model.ver, ".rds"))
}
