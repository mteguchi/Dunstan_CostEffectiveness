...
In this section, I use the entire dataset and estimate the number of hatchlings for the second half of each evening when no survey was conducted from 2400 - 0600, using all available ratios from all years and all sectors. To estimate the uncertainty, I fitted the beta distribution to all available ratios and computed 0.025 and 0.975 quantiles of the fitted beta distribution.  
...
As it was seen from the previous analyses, observed and estimated counts were similar for 2015 and 2016. Departures were observed for 2017 and 2018. The subsequent years have no way of knowing how estimate values compared to the true values (Figure \@ref(fig:Figure-ratio-estimates). 

```{r Figure-ratio-estimates, echo=FALSE, message=FALSE, fig.cap="Estimated abundance and their 95% CI of hatchling counts during the second half of the night when estimated using the proportion of the number of hatchlings during the first half to the entire night. The folid lines indicate the y=x. Hollow squares indicate observed counts."}
...
First, I considered the Poisson distribution as the likelihood function for the observed counts ($n$ and $m$). However, it did not fit well according to the Pareto k statistics, which measure how well a statistical model fits to the observed data in the Bayesian context (ref here). Consequently, I altered the likelihood function to the normal distribution and transformed the observed counts in the natural log scale. In order to deal with zero counts, I added one to each count.
...
jags.data <- list(n = log(counts.1st.2nd.jags.data$counts_1 + 1),
                  ...
                  n.years = length(unique(counts.1st.2nd.jags.data$year_num)),
                  n.sectors = length(unique(counts.1st.2nd.jags.data$sector_num)),
                  N.fem = counts.1st.2nd.jags.data$n.females.scaled,
...
```{r X-validation-1, echo=FALSE, message=FALSE, cache=TRUE}
...
model.ver <- "v1-1"
p.X_validation_V1_1 <- ggplot(cross.validation.v1_1.out$output) +
...
save.fig.fcn(p.X_validation_V1_1, 
...
The model-based estimation process requires a considerate effort. Computing ratios is a lot simpler and more efficient. Here, I compare the two methods and discuss pros and cons of two approaches and recommendations for future sampling effort. 

There was no clear winner in the comparison. Surprisingly, using the simple ratio from 2015-2018 provided "reasonable" estimates of hatchling counts. Estimates were not always accurate, however. Results of the model-based approach were not much different from those from the ratio-based approach. In both approaches, the difference between the estimates and the true counts (when available) was greater when the number of counts during the first half of the evening was larger.

To estimate the total hatchling production on Raine Island, the 6-hr sampling of hatchling may be sufficient when the number of hatchlings, therefore nests, is low. When there is a high nesting season, it may be necessary to conduct a few 12-hr sampling to better estimate the ratio or the correction factor for that year (and others). Because the ratio of the first half to the total appears to be very similar among sectors, only one sector needs to be samples for 12 hrs if 12-hr sampling is logistically challenging. 
