...
$ m_{j,k,y} \sim POI(\lambda_{j,k,y} \times p_{j,k,y}) $.
...
if (!file.exists(out.file.name)){
  model.names <- c("v1-1", "v1-2", "v1-3", "v1-4",
                   "v2-1", "v2-2", "v2-3", "v2-4",
                   "v3-1", "v3-2", "v3-3", "v3-4",
                   "v4-1", "v4-2", "v4-3", "v4-4",
                   "v5-1", "v5-2", "v5-3", "v5-4")
  ...
  k <- 1
  for (k in 18:length(model.names)){
...
flextable(LOOIC.df) %>% 
 ...
 set_caption(paste0("Model comparison.")) %>%
  set_table_properties(width = 0.5, layout = "autofit") %>%
  colformat_double(j = c("DIC", "Rhat", "LOOIC.m", "max.pareto.m", "LOOIC.n", "max.pareto.n", "dDIC"), digits = 2)
...
jm <- out.list$jm.out[[which(out.list$model.names == "v1-2")]]

# for v1-2
mcmc_trace(jm$samples, c("p[1]", "p[2]", "p[3]",
                         "p[4]"))
...
#mcmc_trace(jm$samples, "p")  # for v5-1
```
...
mcmc_trace(jm$samples, c("sigma.n", "B.N.fem"))
...
if (!file.exists(cross.validation.out.file.name)){
  ...
  for (k in 1:nrow(counts.1st.2nd.jags.data)){
    ...
    # Poisson data
    # jags.data <- list(n = x.val.jags.data$counts_1,
    #               m = x.val.jags.data$counts_2,
    #               length = x.val.jags.data$length,
    #               day = x.val.jags.data$n.days,
    #               sector = x.val.jags.data$sector_num,
    #               n.years = length(unique(x.val.jags.data$year_num)),
    #               n.sectors = length(unique(x.val.jags.data$sector_num)),
    #               N.fem = x.val.jags.data$n.females,
    #               year = x.val.jags.data$year_num,
    #               seq_date = x.val.jags.data$seq_date,
    #               max.seq_date = max(x.val.jags.data$seq_date, na.rm = T),
    #               n.data = nrow(x.val.jags.data))

    # log Normal data
    jags.data <- list(n = log(x.val.jags.data$counts_1+1),
                      m = log(x.val.jags.data$counts_2+1),
                      length = x.val.jags.data$length,
                      day = x.val.jags.data$n.days,
                      sector = x.val.jags.data$sector_num,
                      n.years = length(unique(x.val.jags.data$year_num)),
    ...
    counts.X.val$estim.lambda[k] <- exp(jm$mean$lambda[Y,S,D]) - 1
    counts.X.val$q2.5.lambda[k] <- exp(jm$q2.5$lambda[Y,S,D]) - 1
    counts.X.val$q50.lambda[k] <- exp(jm$q50$lambda[Y,S,D]) - 1
    counts.X.val$q97.5.lambda[k] <- exp(jm$q97.5$lambda[Y,S,D]) - 1
