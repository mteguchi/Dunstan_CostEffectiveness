...
# Compute the quantiles of increase for each year - start here 2019-10-29


```

The reprofiled sectors consistently produced more hatchlings than the unmodified South sector over the four year period (Tables 2, 2a, 2b). The increase in hatchling productions in the 2014R sector ranged from ```r signif(min(increase.2014R), digits = 2)``` in ```r colnames(increase.2014R)[increase.2014R == min(increase.2014R)]``` to ```r signif(max(increase.2014R), digits = 2)``` in ```r colnames(increase.2014R)[increase.2014R == max(increase.2014R)]``` (Table 2).  For the 2017R sector, the hatchling production per 100 m was ```r signif(median.100m.wide["2017R", "2017"]/median.100m.wide["South", "2017"], 2)``` times greater than that from the South section.  
