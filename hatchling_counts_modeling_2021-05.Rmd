...
col.def <- cols(Date = col_date(format = "%m/%d/%Y"),
                ...
                Season = col_factor(),
                Sector = col_factor(levels = c("2014R", "2017R", 
                                               "2019R", "North", 
                                               "South", "West")),
...
if (!file.exists(paste0("RData/poly_imputation_dnorm_", 
  ...
  saveRDS(jm, paste0("RData/poly_imputation_dnorm_", 
                     Day1, "_", Day2, "_2021.rds"))
  
} else {
  jm <- readRDS(paste0("RData/poly_imputation_dnorm_", 
                       Day1, "_", Day2, "_2021.rds"))
}
...
obs.t <- obs.y.1 <- matrix(NA, nrow = length(Sectors) * length(seasons), 
                           ncol = 10)

n.vec <- vector(mode = "numeric", 
                length = length(Sectors) * length(seasons))

season.sector.idx <- matrix(NA, nrow = length(Sectors) * length(seasons), 
                            ncol = 2)
...
for (season_idx in 1:length(seasons)){
  for (sector_idx in 1:length(Sectors)){
    data.3 %>% 
      filter(Season == seasons[season_idx]) %>%
...
if (!file.exists(paste0("RData/Girondot_par_estimation_dnorm_", 
                        Day1, "_", Day2, "_2021.rds"))){
  jm <- jags(jags.data,
             inits = NULL,
             parameters.to.save= jags.params,
  ...
  saveRDS(jm, paste0("RData/Girondot_Par_estimation_dnorm_", 
                     Day1, "_", Day2, "_2021.rds"))
