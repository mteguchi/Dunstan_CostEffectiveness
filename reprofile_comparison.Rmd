...
# Control regions
ys.South.2014 <- get.data(data.3.1%>%filter(Sector == "South" | Sector == "A"))
...
mcmc_dens(jm$samples, c("mu2014[1]",  "sigma2014[1]", 
                        "muSouth2014[1]", "sigmaSouth2014[1]"))
```


```{r}
mcmc_dens(jm$samples, c("mu2014[6]",  "sigma2014[6]", 
                        "muSouth2014[6]", "sigmaSouth2014[6]"))
```


```{r}
mcmc_dens(jm$samples, c("mu2017[1]",  "sigma2017[1]", 
                        "muSouth2017[1]", "sigmaSouth2017[1]"))
```
...
ratio.2014.df <- get.samples(pars = "ratio2014", 
                             names = "2014R", 
                             years.2014, jm, n.samples)
years.2017 <- 2017:2020
ratio.2017.df <- get.samples(pars = "ratio2017", 
                          names = "2017R", years.2017, jm, n.samples)
years.2019 <- 2019:2021
ratio.2019.df <- get.samples(pars = "ratio2019",
                          names = "2019R", 
                          years.2019, jm, n.samples)

p.ratio.2014R <- ggplot(data = ratio.2014.df %>% filter(mean < 10), 
       aes(x = (mean), 
           y = as.factor(year), 
           fill = treatment)) +
...
#          Sector = "2014R") %>%
#   select(Year, Sector, X50., X2.5., X97.5., f) -> ratio.2014.df
# 
...
#          Sector = "2017R") %>%
#   select(Year, Sector, X50., X2.5., X97.5., f) -> ratio.2017.df
# 
...
#          Sector = "2019R") %>%
#   select(Year, Sector, X50., X2.5., X97.5., f) -> ratio.2019.df
# 
# # f is the proportion of posterior samples that have the same sign as the mean. 
# 
...
# if (save.fig)
#   ggsave(p.ratio, 
#          filename = "figures/hatchling_ratio.png",
#          device = "png", dpi = 600)

```

```{r}
p.ratio.2017R <- ggplot(data = ratio.2017.df %>% filter(mean < 10), 
                        aes(x = (mean), 
                            y = as.factor(year), 
                            fill = treatment)) +
  geom_vline(aes(xintercept = 1.0)) +
...
```{r}
p.ratio.2019R <- ggplot(data = ratio.2019.df %>% filter(mean < 10), 
                        aes(x = (mean), 
                            y = as.factor(year), 
                            fill = treatment)) +
  geom_vline(aes(xintercept = 1.0)) +
...
# 2014
mu.2014.df <- get.samples(c("mu2014", "muSouth2014"), 
                          c("Reprofile", "Control"), 
                          years.2014, jm, n.samples)
mu.2017.df <- get.samples(c("mu2017", "muSouth2017"), 
                          c("Reprofile", "Control"), 
                          years.2017, jm, n.samples)
mu.2019.df <- get.samples(c("mu2019", "muSouth2019"), 
                          c("Reprofile", "Control"), 
                          years.2019, jm, n.samples)
...
p.2014R <- ggplot(data = mu.2014.df %>% filter(mean < 8.5), 
       aes(x = (exp(mean)), 
           y = as.factor(year), 
           fill = treatment)) +
  geom_density_ridges(bandwidth = 0.5, alpha = 0.6) +
...
```{r}
p.2017R <- ggplot(data = mu.2017.df %>% filter(mean < 8.5), 
       aes(x = (exp(mean)), 
           y = as.factor(year), 
           fill = treatment)) +
  geom_density_ridges(bandwidth = 0.5, alpha = 0.6) +
...
```{r}
p.2019R <- ggplot(data = mu.2019.df %>% filter(mean < 8.5), 
       aes(x = (exp(mean)), 
           y = as.factor(year), 
           fill = treatment)) +
  geom_density_ridges(bandwidth = 0.5, alpha = 0.6) +
