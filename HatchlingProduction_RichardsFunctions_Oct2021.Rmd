...
Raw hatching counts per 100 m transects were compared between 2014R and South sections and between 2017R and South sections. These comparisons indicated that modified sections (2014R and 2017R) produced more hatchlings than the control section (South) in all but one year (2019; Figure \@ref(fig:plot-obs-comparison)). 
...
p.obs <- ggplot(data.3.reprofile) +
   stat_boxplot(aes(x = Season, y = Counts, color = Sector))
  #facet_grid(~ Sector)

if (save.fig)
  ggsave(plot = p.obs,
         filename = "figure/reprofile_comparison.png",
         device = "png", dpi = 600)
...
```{r plot-obs-comparison, echo=F, cache=T, fig.cap = "Observed hatchling counts at three comparable sections. 2014R was modified in 2014, whereas 2017R was modified in 2017. South section was not modified and used as the control section. Horizontal lines are medians, boxes are from 25th and 75th percentiles (inter quartile range; IQR), and whiskers indicate 1.5*IQR."}
...
# ANOVA comparisons
lm.2014R <- lm(Counts_100m ~ Sector * Season,
               data = data.3.2014R)

lm.2017R <- lm(Counts_100m ~ Sector * Season,
               data = data.3.2017R)
#summary(lm.2014R)
